<template>
  ...
  >
    ...
    >
      ...
      <TabBar v-show="isShowTabbar" @delCachePage="delCachePage" />
    </section>
    <div
...
<script lang="ts">
  import { computed, defineComponent, onMounted, unref, ref, onUnmounted } from 'vue'
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const isShowTabbar = computed(() => {
        return layoutStore.state.isShowTabbar
      })
      ...
      const delCachePage = () => {
        ...
        if (name) {
          store.state.cachedView = store.state.cachedView.filter((item) => item != name)
        }
      }
      onUnmounted(() => {
