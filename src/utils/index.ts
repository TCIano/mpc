...
export function mapTwoLevelRouter(srcRoutes: Array<RouteRecordRaw>) {
  function addParentRoute(routes: any, parent: any, parentPath: string) {
    routes.forEach((it: RouteRecordRaw) => {
      if (it.meta && !it.meta.icon) it.meta.icon = 'MenuOutlined'
  ...
  if (srcRoutes && srcRoutes.length > 0) {
    ...
    srcRoutes.forEach((it) => {
      const route = { ...it }
      // route.meta?.icon  = route.meta?.icon ? (it.meta?.icon : 'MenuOutlined')
      // if (!route.name && route.meta?.isSingle && route.children) {
      //   let name = route.children[0].name
      //   route.name = name?.toUpperCase()
      // }
...
export function transfromRoutes(
  originRoutes: Array<RouteRecordRaw | any>,
  parentPath = '/'
  ...
  originRoutes.forEach((it) => {
    const tempRoute = {
      ...it,
      ...(!it.name && it.meta?.isSingle && it.children
        ? { name: it.children[0].name.toUpperCase() } //给单级菜单添加名字
        : {}),
      hidden: (it as RouteRecordRawWithHidden).hidden
        ? !!(it as RouteRecordRawWithHidden).hidden
        : false,
    ...
    .forEach((it) => {
      ...
      if (it.children) {
        if (it.meta && it.meta.isSingle && it.children.length === 1) {
          //给单级菜单中的children进行判断，如果有hiddens则也不显示该子菜单，否则显示子菜单中的“下一页”按钮。
          if ((it.children[0] as RouteRecordRawWithHidden).hidden) return
